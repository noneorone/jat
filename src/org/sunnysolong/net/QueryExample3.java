...
public class QueryExample3 {
  ...
  /**
   * Fill in your Google account email here. The account should be already set up for Google Base.
   */
  ...
  public void displayMyItems(String token) throws MalformedURLException, IOException {
    ...
    if (responseCode == HttpURLConnection.HTTP_OK) {
      ...
      response = ParseError(connection.getErrorStream());
    }
  ...
  /**
   * Parses an XML-format error message.
   * 
   * @param inputStream the error message in XML format
   * @return the description of the error, extracted from the input error message
   * @throws IOException if an I/O exception occurs while reading the message 
   */
  private String ParseError(InputStream inputStream) throws IOException {
    InputSource responeXML = new InputSource(inputStream);
